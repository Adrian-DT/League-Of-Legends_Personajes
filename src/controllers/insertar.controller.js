...
// Controlador que sirve la página insertar.html
controller.anadir = (req, res) => {
    res.sendFile(path.join(__dirname, "../../public/insertar.html"));
}

// Función que me devuelve los personajes
controller.mostrarPersonajes = async (req, res) => {
    const personajes = await fs.readFile(rutaPersonajesJSON);
    res.send(JSON.parse(personajes).personajes);
}
